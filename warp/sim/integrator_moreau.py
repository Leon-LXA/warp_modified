...
def construct_contact_jacobian(
    ...
    for i in range(0, 4): # iterate all contacts
        contact_id = tid*4 + i
        ...
        # add all parents to the jacobian
        body = c_body
        for j in range(0, 4): # iterate all bodies
            parent = joint_parent[body]
            if (parent >= 0):
                contact_jacobian[i*4+j] += joint_S_s[parent]
            body = parent
    ...
    X_s = body_X_sc[c_body]              # position of colliding body
    v_s = body_v_s[c_body]              # velocity of colliding body

    n = wp.vec3(0.0, 1.0, 0.0)

    # transform point to world space
    p = wp.transform_point(X_s, c_point) - n * c_dist  # add on 'thickness' of shape, e.g.: radius of sphere/
...
@wp.func
def prox_iteration(
    x_dot_now: wp.array(dtype=float), # dims gen coord
    m_inv: wp.array, # dims gen coord x gen coord
    h_terms: wp.vec3, # dims gen coord
    jc: mat43, # dims num contacts x gen coord
    mu: float,
    dt: float,
    t_now: float,
    prox_iter: int,
):
    del_mat = jc * m_inv * wp.transpose(jc)
    c = wp.identity(n=4, dtype=float) * jc * x_dot_now + jc * m_inv * h_terms * (dt - t_now)

    # get subvectors of c
    c_1 = wp.vec2(c[0], c[1])
    c_2 = wp.vec2(c[2], c[3])

    # get submatrices of del_mat
    del_mat_11 = wp.mat22(
        del_mat[0 * 2, 0 * 2],
        del_mat[0 * 2, 0 * 2 + 1],
        del_mat[0 * 2 + 1, 0 * 2],
    ...
    p_1 = -wp.inverse(del_mat_11) * c_1  # -G^-1 * c
    p_2 = -wp.inverse(del_mat_22) * c_2  # inverse of 0 matrix is 0 matrix
    # overwrite percussions with steady state only in normal direction
    p_1 = wp.vec2(0.0, p_1[1])
    p_2 = wp.vec2(0.0, p_2[1])

    # solve percussions iteratively
    for prox_iter in range(prox_iter):
        # CONTACT 1
        ...
        # CONTACT 11
        sum = sum + del_mat_11 * p_1
        r_sum = r_sum + wp.determinant(del_mat_11)

        # CONTACT 12
        sum = sum + del_mat_12 * p_2
        r_sum = r_sum + wp.determinant(del_mat_12)
        ...
        if p_1[1] < 0:
            p_1 = wp.vec2(0.0, 0.0)
        p_1 = wp.vec2(wp.clamp(p_1[0], -mu * p_1[1], mu * p_1[1]), p_1[1])
        ...
        # CONTACT 21
        sum = sum + del_mat_21 * p_1
        r_sum = r_sum + wp.determinant(del_mat_21)

        # CONTACT 22
        sum = sum + del_mat_22 * p_2
        r_sum = r_sum + wp.determinant(del_mat_22)

        r = 1.0 / (1.0 + r_sum)  # +1 for stability

        # select subvector of c
        c_i = wp.vec2(c[1 * 2], c[1 * 2 + 1])
        ...
        if p_2[1] < 0:
            p_2 = wp.vec2(0.0, 0.0)
        p_2 = wp.vec2(wp.clamp(p_2[0], -mu * p_2[1], mu * p_2[1]), p_2[1])

    p_contact = wp.vec4(p_1[0], p_1[1], p_2[0], p_2[1])
    return p_contact
...
class SemiImplicitMoreauIntegrator:
    ...
    def simulate(self, model, state_in, state_out, dt, requires_grad=True, update_mass_matrix=False, alpha = 1.0):
        ...
        wp.launch(
            ...
            outputs=[
                state_out.body_ft_s_h,
                state_out.joint_tau
            ],
            device=model.device,
            )

        # check for contacts and construct J_c
        # for each contact point (only foot contact):
        # find jacobian with pen and paper?
        # easy for quadruped
        ...
        # compute G and c
        # Once jacobian is found, G and c should be easy
        # prox iter
        # should be easy
        ...
        # integrate (dflex uses symplectic)
        # difference/why is midpoint not better?
